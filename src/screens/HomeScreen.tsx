...
const HomeScreen = ({ onTabPress }: any) => {
  ...
  const dashboardItems = [
    { title: "My tasks", icon: TodayTaskIcon, color: colors.successGreenDeep, screen: 'MyTasks' },
    { title: 'Learning Section', icon: LearningSectionIcon, color: colors.blue, screen: 'LearningSection' },
              ...
              {announcements.map(item => (
                <TouchableOpacity key={item.id} style={styles.affairCard} activeOpacity={0.7}>
                  {item.photo ? (
                    ...
                    <View style={[styles.affairImage, { backgroundColor: colors.border }]} />
                  )}
                  <View style={styles.affairOverlay}>
                    <View style={[styles.categoryBadge, { backgroundColor: colors.tealDeepOverlay09 }]}>
