...
import { getQASubjects, getQACategories, getQASubcategories, QASubject } from '../services/api';
...
const QASubjectsScreen = ({ navigation }: any) => {
  const [subjects, setSubjects] = useState<QASubject[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    getQASubjects().then(({ data }) => setSubjects(data)).catch(console.error).finally(() => setLoading(false));
  }, []);
          ...
          return (
            <TouchableOpacity
              ...
              onPress={() => navigation.navigate('QACategories', {
                ...
                color,
                bg,
                fetchFn: () => getQACategories({ subject_id: item.id }),
                nextScreen: 'QASubcategories',
                nextParams: (cat: any) => ({
                  title: cat.name,
                  fetchFn: () => getQASubcategories({ category_id: cat.id }),
                  nextScreen: 'QAQuestions',
                  nextParams: (sub: any) => ({ subcategory: sub.name, subcategoryId: sub.id }),
                }),
              })}
            >
...
export default QASubjectsScreen;
